...
export function getWelcomeAccessContext(round, player) {
  const groupNumber =
  ...
  return {
    ...
    isSupervisor,
    isMarshalForGroup,
    shouldSendWelcomeMessage: isSupervisor || isMarshalForGroup,
  };
}
...
export function buildWelcomeAccess(round, player, token) {
  ...
  const { isSupervisor, isMarshalForGroup } = getWelcomeAccessContext(
    round,
    player
  );
  const pathname =
...
export function buildRoundWelcomeGroupMessage({
  campo,
  description,
  startedAt,
  players,
}) {
  const portalUrl = process.env.APP_URL || "https://www.muertelenta.xyz";
  const whatsappJoinUrl = "https://wa.me/5215664089788?text=ingreso";
  const template =
    WELCOME_MESSAGES[Math.floor(Math.random() * WELCOME_MESSAGES.length)];
  const lines = [template.replace("{campo}", campo)];
  ...
  if (roster.length > 0) {
    ...
    roster.forEach((player) => {
      const parts = [];
      if (player?.group && Number.isFinite(Number(player.group))) {
        parts.push(`Grupo ${player.group}`);
      }
      parts.push(player?.name || "Jugador");
      if (player?.handicap != null) {
        parts.push(`HC ${player.handicap}`);
      }
      if (player?.teeName) {
        parts.push(`Tee ${player.teeName}`);
      }
      if (player?.courseHandicap != null) {
        parts.push(`HC Tee ${player.courseHandicap}`);
      }
  ...
  lines.push("");
  lines.push(
    `Puedes seguir la jugada en ${portalUrl} y solicitar tu ingreso por WhatsApp enviando este mensaje: ${whatsappJoinUrl}`
  );
